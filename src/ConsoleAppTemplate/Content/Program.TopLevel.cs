...
// Top-level statements are the entry point - there is no explicit Main. The command
// model (attributes), the --version provider and the default OnExecute live on the
// partial Program class declared below, so the McMaster command framework and the
// AutoRegisterCommandsConvention work exactly as they do in the Main-based variant.
try
{
...
// TODO If you prefer explicit registration, remove the AddConvention call above
